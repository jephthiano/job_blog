...
function content_not_available($type="full"){
	if($type === "short"){
		?>
		<div class='j-gray j-border j-round j-border-gray'style='min-height:120px;margin-bottom: 3px;'><br><br>
				<div class='j-large j-center j-padding'>Content is not available</div><br>
		</div>
		<?php
	}else{
		?>
		<div class='j-light-gray j-border j-border-gray'style='min-height:120px;margin-bottom: 3px;'>
				<div class='j-large j-center j-dark-gray j-padding'>Content is not available</div><br>
				<center>
...
function regex($type,$data){
 if($type === 'email'){
  ...
  $reg = "/^[\w]*\,?\ ?[\w]*\,?\ ?[\w]*\,?\ ?[\w]*\,?\ ?$/";
 }elseif($type === 'word_space'){ //for word and space
  $reg = "/^[a-zA-Z ]*$/";
 }elseif($type === 'word_number_nospace'){ //for word number without space
  $reg = "/^[a-zA-Z0-9]*$/";
 }elseif($type === 'phonenumber'){ //for phonenumber
  $reg = "/^\+?[\d]{11,17}$/";
 }elseif($type === 'skill'){ // for word . ' - @ 
  $reg = "/^[\w .'-@]+$/";
 }elseif($type === 'sql_date'){//for sql date
...
function get_header($header,$button='settings',$size=30){
 if($button === 'settings'){
  $btn = "<span style='margin-right:30px;'class='j-btn j-hide-large j-hide-xlarge'onclick=$('#settings_modal').show();><i class='j-xlarge ".icon('cog')."'></i></span>";
 }elseif($button === 'back'){
 ...
 <div class='j-hide-large j-hide-xlarge'style='margin-bottom:<?=$size?>px;'>
  <div class='j-large j-bolder j-padding j-fixed-top j-card-2 j-color4'><span><?=$btn.$header?></span></div>
  <br>
 </div>
 <?php
}
