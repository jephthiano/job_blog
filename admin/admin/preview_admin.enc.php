				...
				<div class="j-container j-color4 j-padding">
					...
					if($id === false){
						...
						if($adid === 1){
							...
							?>
							<div class='j-right'>
								<span class='j-clickable j-color1 j-btn j-round j-bolder'style="margin:3px;"onclick="$('#update_admin<?=$id?>').show();">
									<i class='<?=icon('sort-amount-up');?>'style='padding-right:5px;'></i>
									Update <?=ucwords($username)?> Level
								</span>
								<span class='j-clickable j-color1 j-btn j-round j-bolder'style="margin:3px;"onclick="$('#admin<?=$id?>status').show();">
									<i class='<?=icon('ban');?>'style='padding-right:5px;'></i>
									<?= $status === 'active'?'Suspend':'Re-activate';?> <?=ucwords($username)?>
								</span>
								<span class='j-clickable j-color1 j-btn j-round j-bolder'style="margin:3px;"onclick="$('#delete_admin<?=$id?>').show();">
