...
if(isset($_GET['page']) AND is_numeric($_GET['page'])){ //getting the value of the get 
	...
	if(!empty($cid)){	
		$id = content_data('subscriber_table','s_id',$cid,'s_id');
	}else{
	...
	<div class="j-col s12 l10"id='mainbody'>
		...
		<div id="maincol"class='j-main-col'>
			<div class=''style='margin-bottom:20px;'>
				<h2 class='j-text-color1 j-padding j-color4'><b>SUBSCRIBER DATA</b></h2>
			</div>
			<center>
				<div class='j-padding'>
					<a href="<?=file_location('admin_url','subscriber/')?>" class="j-bolder j-btn j-color1 j-left j-round j-card-4">Show All Subscriber</a>
			...
			<div class="j-container j-color4 j-padding">
					...
					if($id === false){
						...
						$email = content_data('subscriber_table','s_email',$id,'s_id','','null');
						if($adlevel > 1){
							preview_modal('subscriber',$id);
							?>
							<div class='j-right'>
								<span class='j-clickable j-color1 j-btn j-round j-bolder'style="margin:3px;"onclick="$('#delete_subscriber<?=$id?>').show();">
									<i class='<?=icon('trash');?>'style='padding-right:5px;'></i>
									Delete Subscriber
								</span>
								<a href='<?=file_location('admin_url',"message/send_email/{$email}/")?>'class='j-clickable j-color1 j-btn j-round j-bolder'style="margin:3px;">
									Reply
								</a>
							</div>
							<span class='j-clearfix'></span>
						...
						<div class="j-row-padding j-padding">
							...
							<div class='j-col s8 j-text-color5'><b><?=ucwords((content_data('subscriber_table','s_name',$id,'s_id','','null')))?></b></div>
						</div>
						<div class="j-row-padding j-padding">
							<div class='j-col s4 j-text-color3 j-bolder'><b>EMAIL</b></div>
							<div class='j-col s8 j-text-color5'><?=$email?></div>
						</div>
						<br><br>
						<?php
					}
