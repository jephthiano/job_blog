			...
			<div class="j-responsive"style='line-height:27px;'>
				...
				<table class="j-table-all j-border-0">
					...
					while($stmt->fetch()){
						?>
						<tr class="j-border-0">
							<td><?php s_n();?></td><td><?=ucwords(($name));?></td><td><i class="<?=icon($icon,'fab');?>"></i></td><td><?= text_length2(($link),30);?></td>
							...
							<td><i class="j-large <?= icon('trash');?> j-clickable"onclick="$('#delete_social_handle<?=$id?>').show();"></i></td>
						</tr>
						<?php
