...
if(isset($_POST['s']) && isset($_POST['pg'])){
	...
	if(!empty($searchtext)){ // if the search text is not empty
		...
		$sql = "SELECT s_name,s_email FROM subscriber_table
		...
		$sql = "SELECT s_id,s_name,s_email FROM subscriber_table ORDER BY s_id DESC LIMIT $start,$display";
		$stmt = $conn->prepare($sql);
		$stmt->bindColumn('s_id',$id);
		$stmt->bindColumn('s_name',$name);
		$stmt->bindColumn('s_email',$email);
		...
		<center>
			<div class="j-responsive"style='line-height:27px;'>
				<p class="j-text-color5">
					<b><?=empty($searchtext)?$numRow.' Subscriber(s)':$numRow." result(s) found for <span class='j-text-color1'> '".remove_last_value($searchtext)."' </span> in Subscribers";?></b>
				</p>
